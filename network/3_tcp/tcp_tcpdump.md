...
![提纲](https://cdn.jsdelivr.net/gh/xiaolincoder/ImageHost/计算机网络/TCP-Wireshark/2.jpg)
...
![](https://cdn.jsdelivr.net/gh/xiaolincoder/ImageHost/计算机网络/TCP-Wireshark/3.jpg)

要抓取上面的 ping 命令数据包，首先我们要知道 ping 的数据包是 `icmp` 协议，接着在使用 tcpdump 抓包的时候，就可以指定只抓 icmp 协议的数据包：

![](https://cdn.jsdelivr.net/gh/xiaolincoder/ImageHost/计算机网络/TCP-Wireshark/4.jpg)

那么当 tcpdump 抓取到 icmp 数据包后， 输出格式如下：

![](https://cdn.jsdelivr.net/gh/xiaolincoder/ImageHost/计算机网络/TCP-Wireshark/5.jpg)

![](https://cdn.jsdelivr.net/gh/xiaolincoder/ImageHost/计算机网络/TCP-Wireshark/6.jpg)
...
![tcpdump 常用选项类](https://cdn.jsdelivr.net/gh/xiaolincoder/ImageHost/计算机网络/TCP-Wireshark/7.jpg)
...
![tcpdump 常用过滤表达式类](https://cdn.jsdelivr.net/gh/xiaolincoder/ImageHost/计算机网络/TCP-Wireshark/8.jpg)
...
![](https://cdn.jsdelivr.net/gh/xiaolincoder/ImageHost/计算机网络/TCP-Wireshark/9.jpg)

接着把 ping.pcap 文件拖到电脑，再用 Wireshark 打开它。打开后，你就可以看到下面这个界面：

![](https://cdn.jsdelivr.net/gh/xiaolincoder/ImageHost/计算机网络/TCP-Wireshark/10.jpg)
...
![ping 网络包](https://cdn.jsdelivr.net/gh/xiaolincoder/ImageHost/计算机网络/TCP-Wireshark/11.jpg)
...
![](https://cdn.jsdelivr.net/gh/xiaolincoder/ImageHost/计算机网络/TCP-Wireshark/12.jpg)
...
![](https://cdn.jsdelivr.net/gh/xiaolincoder/ImageHost/计算机网络/TCP-Wireshark/13.jpg)

接着，在终端二执行下面的 curl 命令：

![](https://cdn.jsdelivr.net/gh/xiaolincoder/ImageHost/计算机网络/TCP-Wireshark/14.jpg)

最后，回到终端一，按下 Ctrl+C 停止 tcpdump，并把得到的 http.pcap 取出到电脑。

使用 Wireshark 打开 http.pcap 后，你就可以在 Wireshark 中，看到如下的界面：

![HTTP 网络包](https://cdn.jsdelivr.net/gh/xiaolincoder/ImageHost/计算机网络/TCP-Wireshark/15.jpg)
...
![TCP 流量图](https://cdn.jsdelivr.net/gh/xiaolincoder/ImageHost/计算机网络/TCP-Wireshark/16.jpg)
...
![取消序列号相对值显示](https://cdn.jsdelivr.net/gh/xiaolincoder/ImageHost/计算机网络/TCP-Wireshark/17.jpg)

取消后，Seq 显示的就是真实值了：

![TCP 流量图](https://cdn.jsdelivr.net/gh/xiaolincoder/ImageHost/计算机网络/TCP-Wireshark/18.jpg)

可见，客户端和服务端的序列号实际上是不同的，序列号是一个随机值。

这其实跟我们书上看到的 TCP 三次握手和四次挥手很类似，作为对比，你通常看到的 TCP 三次握手和四次挥手的流程，基本是这样的：

![TCP 三次握手和四次挥手的流程](https://cdn.jsdelivr.net/gh/xiaolincoder/ImageHost/计算机网络/TCP-Wireshark/19.jpg)
...
![四次挥手](https://cdn.jsdelivr.net/gh/xiaolincoder/ImageHost/计算机网络/TCP-Wireshark/20.jpg)
...
![实验环境](https://cdn.jsdelivr.net/gh/xiaolincoder/ImageHost/计算机网络/TCP-Wireshark/21.jpg)
...
![](https://cdn.jsdelivr.net/gh/xiaolincoder/ImageHost/计算机网络/TCP-Wireshark/22.jpg)

其间 tcpdump 抓包的命令如下：

![](https://cdn.jsdelivr.net/gh/xiaolincoder/ImageHost/计算机网络/TCP-Wireshark/23.jpg)

过了一会， curl 返回了超时连接的错误：

![](https://cdn.jsdelivr.net/gh/xiaolincoder/ImageHost/计算机网络/TCP-Wireshark/24.jpg)

从 `date` 返回的时间，可以发现在超时接近 1 分钟的时间后，curl 返回了错误。

接着，把 tcp_sys_timeout.pcap 文件用 Wireshark 打开分析，显示如下图：

![SYN 超时重传五次](https://cdn.jsdelivr.net/gh/xiaolincoder/ImageHost/计算机网络/TCP-Wireshark/25.jpg)
...
![SYN 超时重传两次](https://cdn.jsdelivr.net/gh/xiaolincoder/ImageHost/计算机网络/TCP-Wireshark/26.jpg)
...
![SYN 超时重传](https://cdn.jsdelivr.net/gh/xiaolincoder/ImageHost/计算机网络/TCP-Wireshark/27.jpg)

### 实验二：TCP 第二次握手 SYN、ACK 丢包

为了模拟客户端收不到服务端第二次握手 SYN、ACK 包，我的做法是在客户端加上防火墙限制，直接粗暴的把来自服务端的数据都丢弃，防火墙的配置如下：

![](https://cdn.jsdelivr.net/gh/xiaolincoder/ImageHost/计算机网络/TCP-Wireshark/28.jpg)

接着，在客户端执行 curl 命令：

![](https://cdn.jsdelivr.net/gh/xiaolincoder/ImageHost/计算机网络/TCP-Wireshark/29.jpg)

从 `date` 返回的时间前后，可以算出大概 1 分钟后，curl 报错退出了。

客户端在这其间抓取的数据包，用 Wireshark 打开分析，显示的时序图如下：

![](https://cdn.jsdelivr.net/gh/xiaolincoder/ImageHost/计算机网络/TCP-Wireshark/30.jpg)
...
![](https://cdn.jsdelivr.net/gh/xiaolincoder/ImageHost/计算机网络/TCP-Wireshark/31.jpg)
...
![](https://cdn.jsdelivr.net/gh/xiaolincoder/ImageHost/计算机网络/TCP-Wireshark/32.jpg)
...
![](https://cdn.jsdelivr.net/gh/xiaolincoder/ImageHost/计算机网络/TCP-Wireshark/33.jpg)

接着，在客户端执行如下 tcpdump 命令：

![](https://cdn.jsdelivr.net/gh/xiaolincoder/ImageHost/计算机网络/TCP-Wireshark/34.jpg)

然后，客户端向服务端发起 telnet，因为 telnet 命令是会发起 TCP 连接，所以用此命令做测试：

![](https://cdn.jsdelivr.net/gh/xiaolincoder/ImageHost/计算机网络/TCP-Wireshark/35.jpg)

此时，由于服务端收不到第三次握手的 ACK 包，所以一直处于 `SYN_RECV` 状态：

![](https://cdn.jsdelivr.net/gh/xiaolincoder/ImageHost/计算机网络/TCP-Wireshark/36.jpg)

而客户端是已完成 TCP 连接建立，处于 `ESTABLISHED` 状态：

![](https://cdn.jsdelivr.net/gh/xiaolincoder/ImageHost/计算机网络/TCP-Wireshark/37.jpg)

过了 1 分钟后，观察发现服务端的 TCP 连接不见了：

![](https://cdn.jsdelivr.net/gh/xiaolincoder/ImageHost/计算机网络/TCP-Wireshark/38.jpg)

过了 30 分钟，客户端依然还是处于 `ESTABLISHED` 状态：

![](https://cdn.jsdelivr.net/gh/xiaolincoder/ImageHost/计算机网络/TCP-Wireshark/39.jpg)

接着，在刚才客户端建立的 telnet 会话，输入 123456 字符，进行发送：

![](https://cdn.jsdelivr.net/gh/xiaolincoder/ImageHost/计算机网络/TCP-Wireshark/40.jpg)

持续「好长」一段时间，客户端的 telnet 才断开连接：

![](https://cdn.jsdelivr.net/gh/xiaolincoder/ImageHost/计算机网络/TCP-Wireshark/41.jpg)
...
![](https://cdn.jsdelivr.net/gh/xiaolincoder/ImageHost/计算机网络/TCP-Wireshark/42.jpg)
...
![](https://cdn.jsdelivr.net/gh/xiaolincoder/ImageHost/计算机网络/TCP-Wireshark/43.jpg)
...
![常规 HTTP 请求](https://cdn.jsdelivr.net/gh/xiaolincoder/ImageHost/计算机网络/TCP-Wireshark/44.jpg)

在 Linux 3.7 内核版本中，提供了 TCP Fast Open 功能，这个功能可以减少 TCP 连接建立的时延。

![常规 HTTP 请求 与 Fast  Open HTTP 请求](https://cdn.jsdelivr.net/gh/xiaolincoder/ImageHost/计算机网络/TCP-Wireshark/45.jpg)
...
![TCP Fast Open 抓包分析](https://cdn.jsdelivr.net/gh/xiaolincoder/ImageHost/计算机网络/TCP-Wireshark/46.jpg)
...
![快速重传机制](https://cdn.jsdelivr.net/gh/xiaolincoder/ImageHost/计算机网络/TCP-Wireshark/47.jpg)

TCP 重复确认和快速重传的一个案例，用 Wireshark 分析，显示如下：

![](https://cdn.jsdelivr.net/gh/xiaolincoder/ImageHost/计算机网络/TCP-Wireshark/48.jpg)
...
![理想状态下的窗口变化](https://cdn.jsdelivr.net/gh/xiaolincoder/ImageHost/计算机网络/TCP-Wireshark/49.jpg)
...
![服务端繁忙状态下的窗口变化](https://cdn.jsdelivr.net/gh/xiaolincoder/ImageHost/计算机网络/TCP-Wireshark/50.jpg)
...
![窗口大小在收缩](https://cdn.jsdelivr.net/gh/xiaolincoder/ImageHost/计算机网络/TCP-Wireshark/51.jpg)
...
![零窗口 与 窗口探测](https://cdn.jsdelivr.net/gh/xiaolincoder/ImageHost/计算机网络/TCP-Wireshark/52.jpg)
...
![](https://cdn.jsdelivr.net/gh/xiaolincoder/ImageHost/计算机网络/TCP-Wireshark/53.jpg)
...
![](https://cdn.jsdelivr.net/gh/xiaolincoder/ImageHost/计算机网络/TCP-Wireshark/54.jpg)
...
![禁用 Nagle 算法 与 启用 Nagle 算法](https://cdn.jsdelivr.net/gh/xiaolincoder/ImageHost/计算机网络/TCP-Wireshark/55.jpg)
...
![关闭 Nagle 算法](https://cdn.jsdelivr.net/gh/xiaolincoder/ImageHost/计算机网络/TCP-Wireshark/56.jpg)
...
![TCP 延迟确认](https://cdn.jsdelivr.net/gh/xiaolincoder/ImageHost/计算机网络/TCP-Wireshark/57.jpg)


延迟等待的时间是在 Linux 内核中定义的，如下图：

![](https://cdn.jsdelivr.net/gh/xiaolincoder/ImageHost/计算机网络/TCP-Wireshark/58.jpg)

关键就需要 `HZ` 这个数值大小，HZ 是跟系统的时钟频率有关，每个操作系统都不一样，在我的 Linux 系统中 HZ 大小是 `1000`，如下图：

![](https://cdn.jsdelivr.net/gh/xiaolincoder/ImageHost/计算机网络/TCP-Wireshark/59.jpg)
...
![关闭 TCP 延迟确认](https://cdn.jsdelivr.net/gh/xiaolincoder/ImageHost/计算机网络/TCP-Wireshark/60.jpg)

>  延迟确认 和 Nagle 算法混合使用时，会产生新的问题

当 TCP 延迟确认 和 Nagle 算法混合使用时，会导致时耗增长，如下图：

![TCP 延迟确认 和 Nagle 算法混合使用](https://cdn.jsdelivr.net/gh/xiaolincoder/ImageHost/计算机网络/TCP-Wireshark/61.jpg)
...
![](https://cdn.jsdelivr.net/gh/xiaolincoder/ImageHost/%E8%AE%A1%E7%AE%97%E6%9C%BA%E7%BD%91%E7%BB%9C/TCP-Wireshark/62.png)
